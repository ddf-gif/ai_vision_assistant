...
| # | 技巧 | 思路 | 最终决策 |
|---|---|---|---|
| 1 | **意图过滤** | 用户说"你好"时不发画面，说"这是什么"才发 | ✅ 采用 |
| 2 | **动态帧率** | 对话活跃时 1fps，闲时 0.1fps | ✅ 采用 |
| 3 | **闲置降频** | 超 30 秒无语音自动降低帧率 | ✅ 采用 |
| 4 | **静音休眠** | 长时间无语音完全关闭摄像头采集 | ✅ 采用 |
| 5 | **音频先于视频** | 视频帧仅在首次音频发送后才推送 | ✅ 采用 |
| 6 | **半双工回声抑制** | AI 说话时丢弃麦克风数据，防止回声循环 | ✅ 采用 |
| 7 | **能量门控** | AI 说完后检测麦克风确认扬声器静音才恢复拾音 | ✅ 采用 |
| 8 | **图像压缩** | 降低分辨率或 JPEG 质量减少带宽 | ❌ 未采用——当前 640×480 + JPEG 80% 已足够小（约 10KB/帧），再降影响识别 |
| 9 | **服务端 VAD** | 依赖模型侧语音检测，客户端不判断 | ❌ 未采用——服务端 VAD 有延迟，客户端预判可省去无效音频的上行带宽 |
